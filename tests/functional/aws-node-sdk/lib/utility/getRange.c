...
# for size in 0 1 2 7 8 9 15 16 17 18 29; do ./knownfile --size=$size toto.$size ; for offset in 1 2 3 4 5 6 7 8 ; do if [ $offset -lt $size ] ; then dd if=toto.$size of=toto.$size.$offset bs=1 skip=$offset 2>/dev/null; if ./knownfile --size=$(($size-$offset)) toto.$size.$offset --check --offset=$offset; then rm -f toto.$size.$offset; else echo Oops toto.$s.$offset ; fi ; fi; done; done
*/

int usage(const char *reason)
{
    fprintf(stderr,
"Usage: knownfile [-h] [--check] [--size SIZE] [--offset OFFSET] [--quiet] filename\n"
"\n"
...
int kf_check(const char *filename, uint64_t size, uint64_t offset)
{
    ...
    if (sb.st_size != size) {
        quietprint("size check failed on \"%s\". Expected %lu, got %lu", filename,
            size, sb.st_size);
        goto end;
    }
    ...
    for (cur = 0; cur < sb.st_size; cur++) {
        if (ptr[cur] != offset2val(offset + cur)) {
            quietprint("Content check failed at offset %lu in  \"%s\", offset %lu in knownfile", cur, filename, offset + cur);
            goto end;
        }
    }
    ret = 0;

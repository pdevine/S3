...
function _validateWebsiteConfigXml(jsonResult, xml, log, cb) {
    ...
    if (resultConfig.ErrorDocument) {
        ...
        _validateStringNotEmpty(parent[0], 'Key', 'InvalidArgument');
    ...
    if (resultConfig.RoutingRules) {
        ...
        for (let i = 0; i < parent[0].RoutingRule.length; i++) {
            ...
            if (rule.Redirect[0].HostName) {
                _validateStringNotEmpty(rule.Redirect[0], 'HostName',
                'InvalidArgument');
                // TODO: evaluate if I want a default err type for that func
                ruleObj.redirect.hostName = rule.Redirect[0].HostName[0];
            }
            if (rule.Redirect[0].ReplaceKeyPrefixWith) {
                _validateStringNotEmpty(rule.Redirect[0],
                    'ReplaceKeyPrefixWith', 'InvalidArgument');
                ruleObj.redirect.replaceKeyPrefixWith =
                    rule.Redirect[0].ReplaceKeyPrefixWith[0];
            }
            if (rule.Redirect[0].ReplaceKeyWith) {
                _validateStringNotEmpty(rule.Redirect[0], 'ReplacePrefixWith',
                    'InvalidArgument');
                ruleObj.redirect.replaceKeyWith =
                    rule.Redirect[0].ReplaceKeyWith[0];
            }
            if (rule.Redirect[0].HttpRedirectCode) {
                _validateStringNotEmpty(rule.Redirect[0], 'HttpRedirectCode',
                'InvalidArgument');
                ruleObj.redirect.httpRedirectCode =
                    rule.Redirect[0].HttpRedirectCode[0];
            }

...
export default function getMultipartUploadListing(bucket, params, callback) {
    ...
    // We need to reset `response.MaxKeys` otherwise it will equal the value of
    // `uploads.length`, when it should instead be the value from the
    // original MaxUploads specified by the user (or the default 1000). Also,
    // `response.MaxKeys` is 0, `response.IsTruncated` should be false.
